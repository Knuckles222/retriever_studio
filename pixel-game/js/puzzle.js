...
const PUZZLE = (() => {
  ...
  const OFFSET_X    = 30;          // 그리드 왼쪽 여백
  const OFFSET_Y    = 24;          // 그리드 위쪽 여백
  ...
  // 방 처음부터 재시작 (게임오버/이동 소진 시 내부에서도 호출)
  function _reset() {
    px           = START_X;
    py           = START_Y;
  ...
  function _drawGrid() {
    ...
    for (let row = 0; row < GRID_ROWS; row++) {
      for (let col = 0; col < GRID_COLS; col++) {
        ...
        fill(c[0], c[1], c[2]);
        rect(OFFSET_X + col * CELL, OFFSET_Y + row * CELL, CELL, CELL);
        ...
        if (tile === TILE.FILE) {
          ...
          text('FILE', OFFSET_X + col * CELL + CELL / 2,
                        OFFSET_Y + row * CELL + CELL / 2);
        } else if (tile === TILE.VIRUS_F) {
          fill(255, 200, 200);
          textSize(9);
          textAlign(CENTER, CENTER);
          text('불', OFFSET_X + col * CELL + CELL / 2,
                     OFFSET_Y + row * CELL + CELL / 2);
        } else if (tile === TILE.VIRUS_W) {
          fill(200, 230, 255);
          textSize(9);
          textAlign(CENTER, CENTER);
          text('물', OFFSET_X + col * CELL + CELL / 2,
                     OFFSET_Y + row * CELL + CELL / 2);
        } else if (tile === TILE.VIRUS_G) {
          fill(200, 255, 200);
          textSize(9);
          textAlign(CENTER, CENTER);
          text('풀', OFFSET_X + col * CELL + CELL / 2,
                     OFFSET_Y + row * CELL + CELL / 2);
        }
  ...
  function _drawPlayer() {
    ...
    const x = OFFSET_X + px * CELL;
    const y = OFFSET_Y + py * CELL;
  ...
  // ── HUD (이동 횟수 / 속성 표시) ────────────────────────
  function _drawHUD() {
    const hudX = OFFSET_X + GRID_COLS * CELL + 20;  // 그리드 오른쪽
    ...
    fill(255, 220, 50);
    textSize(28);
    text(movesLeft, hudX, 118);

    // 조작 안내
    fill(140);
    textSize(11);
    text('방향키로 이동', hudX, 200);
    text('↑ ↓ ← →', hudX, 218);
    ...
    textSize(10);
    text('─────────────', hudX, 260);
    text('불  = 빨강 통과', hudX, 278);
    text('물  = 파랑 통과', hudX, 293);
    text('풀  = 초록 통과', hudX, 308);
    text('흰색 = 통과 불가', hudX, 323);
  }
